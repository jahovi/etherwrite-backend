...
export default class DLList {
	...
	*/
	 ...
	 */
	public insertAfterCurrentAndMoveCurrent(val: string, author: string): void {
		this.insertAfterCurrent(val, author);
		if (this.current.next)
			this.current = this.current.next;
	}
	 ...
	 * @param author the id of the author
	 */
	public insertAfterCurrent(val: string, author: string): void {
		if (val.length != 1) {
